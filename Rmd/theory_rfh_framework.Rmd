...
Outliers can have severe and unwanted impact on predictions. This is true for
the case of linear mixed models but also for hierarchical Bayes; both
methodologies are considered for area level models. In Section
\ref{sec:robustdiscussion} a literature review of robust area level models can
be found. In this context it is noteworthy that these studies focused on how 
outliers can be dealt with in the context of hierarchical Bayes. In the 
following the robust estimation methodology by @Sin09 is being adapted to area 
level models which leads to outlier robust predictions under mixed linear 
models. Before the robust methods are discussed the different scenarios in which
we may be interested in using robust methods are layed out; i.e. what are
outliers on the area level?
...
outlying observations and furthermore illustrate that it may be possible to
distinguish between unit and area level outliers. In the following sections
these types are characterised to motivate the study of robust area level models.
...
because these estimates become an index of reliablility. For a given sample size
...
direct estimator. In this case the Fay-Herriot model weights down unreliable 
direct estimators, which is why this model may be able to self\hyp{}adjust for
unit level outliers. This property is explored in section (?) using model based
simulations.
...
effect, $\re_i$. From a model perspective where we treat the random effect as an
i.i.d. random variable following a normal distribution such values may arise by
chance alone. This would mean that in truth the distribution is indeed normal
but we are in one of those rare cases in which we observe abnormal behaviour of
single domains.

This argumentation seems to be unrealistic for applications in SAE since under
repeated sampling we would expect the same domains to be abnormal. This
stimulates a more fundamental discussion about linear mixed models in Small Area
...
that we should not care about unstable variance estimation. Althogh the
...
Furthermore the proposed Newton\hyp{}Raphson algorithm for the random effects
was surprisingly unstable for area level models which manifested itself in
thousands of iterations until numerical convergence was reached. A possible
reason may be the ratio between observations and random effects, e.g. we have
$D$ areas and make $D$ predictions. However a conclusive analysis of these
problem cannot be provided.
