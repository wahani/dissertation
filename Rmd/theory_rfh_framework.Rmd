...
is notworthy that these studies focused on how to adress outliers in the context
of hierarchical Bayes. In the following I will adapt the robust estimation
methodology by @Sin09 to area level models which leads to outlier robust
predictions under mixed linear models. Before I come to the robust methods I
want to point out different scenarios in which we may be interested in robust
methods; i.e. what are outliers on the area level?
...
indication which type of outlying observations area being observed and the 
existing literature mainly focuses on dealing with area level outliers since
they lead to a simple extension to the FH model - see for example @Dat95 and
@Xie07.
...
turn may be used as the true or rather given variance parameters in a 
Fay-Herriot model.
...
assume that such values can be used as the *true* variance parameters in the 
Fay-Herriot model. Such estimators will have poor properties when the target is 
the *true* sampling error especially in the context of outliers. This problem 
...
*outlying estimates of the standard errors*, which in turn may have unwanted 
...
for all them. This will raise the question if a different distribution should be
used to approximate the random effect, e.g. a mixture distribution of normals or
a scewed distribution.
...
can be influenced by such observations, allthough it is unclear whether they are
under or over estimated. Looking at the EBLUP under the FH model in \eq{eq:fheblup}:
\empty{
\begin{align*}
\tilde{\theta}_i^{FH} = 
...
This will reduce the beneficial effect for the predictions in terms of MSPE
since we rely more on the direct estimates.


## The Creation of Overly Influencial Observations
\label{sec:overly-influencial-observations}

A third kind of outlying observations can be thoght of when we frame them as 
*overly influencial observations*. They are not necessarily far away from the
center of the data. Furthermore they can not be understood by looking at the 
direct estimator alone but must be seen as pairs of direct estimator and 
sampling variance. Together they determine the impact of a single observation on
the overall predictions under a FH model.
...
Both scenarios are situation in which we have unstable variance estimates and
the FH model may not yield stable and reliable results. Especially the second
scenario should not be misunderstood as an argument against smoothing, or that
we should not care about unstable variance estimation. Allthogh the variances
are not smoothed the influence of observations is bounded in a robust estimation
procedure. In fact the introduced robust area level model can be considered to
have a positive effect with respect to both scenarios because single
observations are restricted in their influence. Furthermore the robust model may
present an alternative to smoothing strategies, although that has not been the
initial intention, without the danger of creating overly influencial
observations.
