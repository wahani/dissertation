...
In SAE mixed models are generally decided into area and unit level models. In
...
\eq{eq:fh} can be directly viewed as a linear mixed model as it was introduced in
...
unknown variance parameters in this case is a scalar, such that $\delta = \sigre$
since $\sige$ is assumed to be known. The BLUP under the Fay-Herriot model,
defined in equation \eq{eq:blup}, can then be obtained by setting
...
For the estimation of $\sigre$ different approaches exist. @Fay79 proposed a
moment estimator from which they derived an algorithm to estimate $\sigre$.
...
@Fay79 did not asses the quantification of uncertainty associated with their
...
shrinking both means and variances. Another approach is to stabilise the
...
transformation is used. @Fay79 log\hyp{}transform the direct estimator and
...
often means a dramatic reduction in the number of observations, hence more 
...
Fay-Herriot model explicitly is subject to numerous extensions. See for example
...
components, the random effects or the error term. The use of correlated random
...
investigated the possibility to incorporate a simultaneous autoregressive process
...
space. SAR processes do not have this restriction and are more useful to 
...
the random effect. $u_{1i}$ now follows a simultaneous autoregressive process of
...
parameters. Replacing the unknown parameters with their receptive estimates
...
models and small area predictions. @Por14 is an example how to use a CAR process
...
principle promising for applications. However, an important remark and result
of the reviewed literature is, that modelling spatial autocorrelation as a random
