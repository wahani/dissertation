...
and some diterministic auxiliary variables $\si{x}$:
$$
\si{\theta} = \si{x}^\top \beta + \si{\re}
$$
where $\si{x}$ is a vector containing area-level (aggregated) information for
$P$ variables and $\beta$ is a vector ($1\times P$) of regression coefficients.
The model errors $\si{\re}$ are assumed to be independent and identically
distributed following normal distribution: $\re_i \sim \Distr{N}(0, \sigre)$.
Furthermore $e_i$ and $\re_i$ are assumed to be independent. Combining the 
sampling and linking model leads to:
\begin{align}
\tilde{y}_i = x_i^\top \beta + \re_i + e_i \label{eq:FH}
\end{align}
...
$$
\si{\theta} = \si{\gamma}\si{\tilde{y}} + \Paran{1 - \si{\gamma}} \si{x}^\top\beta
$$
$\si{\gamma} = \sigre / (\sigre + \sige)$
$$
\si{\hat{\theta}} = \si{\hat{\gamma}}\si{\tilde{y}} + \Paran{1 - \si{\hat{\gamma}}} \si{x}^\top\hat{\beta}
$$
$\si{\hat{\gamma}} = \hat{\sigma}_\re^2 / (\hat{\sigma}_\re^2 + \sige)$
