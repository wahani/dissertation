...
We compare here two quantities, the Monte Carlo RRMSPE and Monte Carlo RBIAS. 
These two measures are computed according to Section (?). The methods that 
performence is studied are the non\hyp{}robust predictions under the FH model 
and robust predection refered to as RFH. To give a concise overview of the
...
- RM -- denotes a robust direct estimator. Here we consider the sample median.
Also a M-type estimator has been investigated with similar results.
...
variance: $\sige \coloneqq \frac{s_i^2}{n_i}$.
- RFH.SM -- is the robust prediction using the sample mean. The same setup is
used as for FH.SM.
...
- RFH.SM.GVF -- is the robust prediction using a generilsed variance function.
This has the same setup as the FH.SM.GVF.
...
sampling variance an estimator is used based on the median absolute deviation 
(MAD): $\sige \coloneqq \frac{\text{MAD}_i^2}{n_i}$.
- RFH.RM -- is the robust prediction based on the sample median. The sampling
variances are estimated as for the FH.RM method.

It would have been possible to investigate more settings than the above. However there is a trade off between the amount of models which can be consistently presented and what we understand. abalabla.

## Results
\label{sec:results_unit_results}

To summarise the results of this study consider Figure \ref{fig:unit_mse} and
\ref{fig:unit_bias}. To structure the discussion note the following observations
we can make:
