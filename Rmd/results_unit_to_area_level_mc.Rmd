...
However results for robust area level models are typically produced on area level
...
estimators are used as an alternative to a sample mean.

These questions have more practical relevance and are conceptually unrelated to
...
excludes the spatial and temporal extensions. The results should in principle 
apply also to these extensions however this remains an avenue for further
research.

This study is structured as follows: the following Section (?) completes the
formal link between the unit and area level. Essentially we need to specify
assumptions with respect to the unit level population model to have a coherent
link between the population level, the sampled level, and the area level. Based
on these results Section (?) presents the simulation settings; Section (?) the
investigated estimation strategies; and in Section (?) the results of the Monte
Carlo Simulation are presented.


## From Unit to Area Level Data
