...
connected to the application of smoothing techniques -- see Section
\ref{sec:theory_sae_fh_mspe} for a review -- to stabilise these parameters.
Hence in this context several points motivate this study:
...
The study is structured as follows: the following Section 
...
Two measures are utilised to asses the quality of predictions: the Monte Carlo 
...
as RFH. The following modelling strategies are considered:
...
- There is only a small advantage of using generalised variance functions as an
...
parameters can be estimated. However he also notes that there is no theory 
supporting this claim. The discussion in Section \ref{sec:theory_sae_fh_mspe}
showed that despite this lack of theory the use of generalised variance
functions is often preferred in practical applications.  Hence it may well be
that we observe results specific to this simulation setting.
...
bias. In Section \ref{sec:overly-influencial-observations} this effect was
labelled as the *creation of overly influential observations*.
...
effect as it was described in Section \ref{sec:unit-level-outliers}. The
prediction mechanism under the FH model gives more weight to the linear
predictor for the outlying domains since we observe a largeer sampling variance
for these type of outliers. This is a desirable property as long as the linear
prediction is a good fit for these observations. When we compare these results
with the non\hyp{}symmetric outlier scenario -- *(0, e)* -- we can see that the
effect of unit level contamination on the prediction is small overall. This is
so for all modelling strategies except when using GVFs for the reasons outlined
above. However in these situations we introduce a bias for the outlying domains.
In contrast to the bias correction for unit level models -- see Section
...
because we create overly influential observations which from their design do not
...
In practice it may be desirable to use a bootstrap for estimating the variance 
of the sample median. With these small samples the results have been 
discouraging and have not lead to an improvement for the area level predictions.
Hence the MAD seemed to provide a sensible alternative. Third, it may be argued 
that a different robust method should be considered. A M-type estimator for the 
sample mean in conjunction with a robust variance estimator has also been 
considered. Very similar results compared to the median have been obtained thus
...
creation of overly influential observations as they can occur after using GVFs
to smooth the sampling variances. However a more thorough investigation of the
use of Gaffs may be necessary to come to a recommendation since the simulation
...
model has not showed to outweigh this effect here.
