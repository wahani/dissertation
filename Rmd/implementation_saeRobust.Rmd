...
level models, e.g. the BHF model of Section \ref{sec:unit_level}, as well as the
non\hyp{}robust spatial- and temporal extensions to the FH model reviewed in
Section \ref{sec:theory_sae_fh_extensions}. Other packages focus on the
implementation of single estimators, e.g. `saery` \citep{Est14} implements the
EBLUP of \eq{eq:tfh} by @Rao94. Robust methods for the SAE field are
...
Also the methods and advances reviewed in Section \ref{sec:robustee} have not
been published in terms of software -- except the results by @Schoch12. With
this respect `saeRobust` aims to provide a first but stable version implementing
the models under consideration. It also may prove to be useful as a vessel for robust
unit level models as most software components are designed to be reused. E.g.
the robust score functions, the proposed algorithms for the regression
coefficients and random effects, as well as more general features like the
pseudo linear representation of a robust estimator are directly reusable as
functions.
...
the ideas in @Wei14[87 ff] and @Zie74 which have been invaluable in creating a
...
the choice of these values. In one of the scenarios we can then see if and how 
...
statistical properties of the estimators are investigated separately in Section
(?).
...
\ref{sec:rfh_solutions} are given here.
...
the spatial and temporal extensions are bounded between $10^{-5} - 1$ and 
$1 - 10^{-5}$; and all variance parameters have a lower bound of $10^{-5}$ to
avoid zero or negative variance parameters -- see also the discussion in
@Rao15[151 ff]. These restrictions are mostly relevant in optimisations with bad
starting values and mainly prevent software failure. In these situations they
also enable the algorithm to find solutions at all.
...
one of the stopping rules given in \ref{sec:rfh_algorithm}. That means we
observe an overall number of iterations and a number of iterations for each
nested algorithm. Hence we have two maximum numbers of iterations as additional
stopping rules: One for each nested algorithm and one for the overall
optimisation.

The value for $\epsilon$ in the convergence criterion given in Section
\ref{sec:rfh_algorithm} is being set to $10^{-6}$ to guarantee sufficient
numerical accuracy.
...
testing. The variance parameters in all models are set to $100$ and correlation 
...
during model fitting. The number of domains is set to $D = 40$ and the number of
time periods -- in scenarios with a time dimension -- is set to $T = 10$. 
- *Outlier*:
...
when studying robust methods -- see the corresponding Section ?
...
\eq{eq:reblupre}. The tuning constant, $k$, for the influence function is fixed
at $1.345$.
...
realisations of these values are non\hyp{}informative and the figures should be
used as a descriptive tool to evaluate the solutions.
...
values close to zero. With this regard the IRWLS algorithm shows acceptable
...
in the data and that explains this effect.
...
iteration. This can be explained by starting from non\hyp{}robust starting
values which may result in large absolute values for the intercept. We can also
see that the algorithm of the variance parameter takes approximately $50$
iterations in the first overall iteration. In some cases even up to $100$,
...
no notable difference between the two scenarios; in both cases many iterations
are needed due to the bad starting values.
...
$\sigre$, and the median random effect. This appears to be different for the
outlier scenario. One possible explanation is that the prediction of the random 
effect compensates the over estimation of the intercept -- thus we observe 
negative median values of the random effects. 
...
values and the corresponding values of the estimation equation \eq{eq:rfheere}
which -- after all -- is what should be close to zero at its solution. What we
observe is that for most domain predictions the value of the estimation equation
is close to zero after few iterations. Only for some domains better solutions
are searched for. This effect becomes stronger with larger number of domains.
This suggests to investigate the solution for the random effects, i.e. the
values of the estimation equation, already after a few iterations especially
when computation time is relevant.
...
As said before the overall algorithm has a maximum number of iterations of $100$
as well as each nested algorithm -- referred to as the *100 Iter* strategy. This 
is a setting suboptimal with respect to the number of iterations however it is 
one which proofed to be without a single software failure during the simulation.
Figure \ref{fig:stability_convergence_spatial} illustrates the choice when we 
set the maximum number of iterations of each nested algorithm to one -- referred
to as the *1 Iter* strategy. It is counter intuitive that the *100 Iter*
strategy needs a lot more iterations because the figure compares the results of
each overall iteration. In fact it needs $1160$ iterations to find a solution
for the variance and $160$ for the correlation parameter compared to $100$ for
the *1 Iter* strategy.
...
algorithm can be unstable when we have bad starting values. 

The *1 Iter* strategy needs often fewer iterations which is a trade\hyp{}off
with a software failure rate of approximately 10 per cent in these testing
scenarios. In preliminary tests it often proofed to be useful to set the maximum
number of iterations at some small number, say $5$, to achieve a balance between
the two scenarios. For applications this suggests that even if the algorithm
fails a different setting of the number of iterations may still provide results.
...
extensions respectively. A main overall effect is that the
...
we do not consider area level outliers; under area level outliers observations
belonging to one domain are expected to be abnormal jointly. However the testing
framework only sets single observations to $10000$. Hence these results may be
artificial in their statistical properties.
...
key to computationally less demanding solutions are the choice of starting
values and the restriction of the maximum number of iterations of the nested 
algorithms. To address this trade-off the implementation in `saeRobust` allows
to set both parameters as well as the number of iterations for the optimisation
...
same parametric bootstrap method as used for estimating the MSPE for the domain
predictions to construct confidence intervals. With respect to model selection
the actual log-likelihood function for robust methods is generally unknown since
we focused on finding robust versions of its partial derivatives. Hence it is
not clear yet how to provide information criteria for the robust methods under
consideration. See also the discussion in @Kol16 a closely related software
implementation for robust mixed linear models.
