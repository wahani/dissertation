...
effects, as well as more general features such as the pseudo linear
...
As stated earlier the current implementation may lack some software features 
which should be available during a data analysis. In this regard some open
research questions and remarks are mentioned in Section
...
zero or negative variance parameters -- see also the discussion in
@Rao15[151 ff]. These restrictions are mostly relevant in optimisations with bad
starting values; and they mainly prevent software failures. In these situations
they also enable the algorithm to find solutions at all.
...
one of the stopping rules given in Section \ref{sec:rfh_algorithm}. This means
...
yielding non robust starting values. All variance parameters are set to $1$ and 
correlation parameters to $0.01$. Starting values for the random effects are
...
the variance with a non robust method we would tend to reach values close to
$10^5$.
...
iterations in both scenarios is $13$ and in most cases few iterations are 
needed.
...
variance of $100$. Two effects can be seen similar to what was discussed earlier
First in the base scenario we estimate variance parameters close to zero which
coincides with larger values for the estimation equation as before. Furthermore
we observe a higher value for the variance parameter and a lower value for the
correlation parameter in the outlier scenario. What happens is that the
additional variation due to the outliers is captured by the variance component
and the spatial correlation structure is somewhat shadowed.
...
needs $1160$ iterations to find a solution for the variance and $160$ for the
correlation parameter compared to $100$ for the *1 Iter* strategy.
...
the correlation parameter is fixed at its initial value of $0.01$. In each
...
the spatial and temporal extensions which can be -- seen in Table 
...
parameter in the AR(1) shows very promising behaviour. Although it is based on a
numeric approximation of the derivative of the estimation equation and uses a 
Newton-Raphson algorithm convergence is reached rapidly. The main reason may be 
the restriction of the parameter space to $\rho_2 < |1|$ which makes it more 
robust against the choice of starting values. This would suggest that a 
Newton-Raphson algorithm may also be useful for optimising the correlation 
parameter in the SAR(1); however this has not been further investigated.
...
reliance on the results. For this reason the current version of `saeRobust`
