...
purpose I adapt these examples.
...
\noindent What we can observe is that the intercept and estimated variance 
parameter are estimated at a higher value than before. This is an indicates that
outlying observations may have an effect on the parameter parameter estimation.
...
correction has no impact on the overall presentation.
...
The above presented methods, that is the `mse`, `predict`, and `plot` methods, 
...
package `nlme` @Pin16 for nonlinear mixed effects models. 
...
the previously used methods but is omitted as their are no new features of
...
This can be done by using the `mse` and specifying the `type` argument. This is 
not done by default since the computation can be timeconsuming. As an example
for such a call consider the following code necessary:
...
robust methods under consideration -- see the discussion in @Kol16.
...
important aspect of this Thesis are the accompanying software
