...
muniscipalities in Tuscany. The target variable is the number of hecta used for 
...
agriculture within each muniscipality: `area`; and the average number of working
days in the reference year (2000): `workdays`. Additionally we have the
proximity matrix of these muniscipalities stored in `grapesprox`. 
...
is in direct corespondence to a linear model in `R` with the additional 
...
the algorithm needed 31 iterations. Each nested algorithm has been retricted to
10 iterations. The optimiser for the random effects needed 9 iterations. With
ragard to the algorithm the user has the option to specify starting values, the
...
\noindent and obtain the Quantile\hyp{}Quantile plots in Figure \ref{fig:saeRobust_residuals_grapes}. Since the FH model and all extensions under consideration assume a heteroscedastic sampling distribution it is not meaningful to plot the estimated sampling errors -- here denoted as residuals. Thus the standardised residuals are used instead:
$$
...
Figure \ref{fig:saeRobust_residuals_grapes} shows here deviation from the normal distribution for residuals and random effects. In both plots we see more extreme values than we should expect; with regard to the random effects this may be a situation in which the robust estimation technique may be beneficial. We can estimate the non\hyp{}robust version by setting the tuning constant to some large value:
...
parameter are estimated at a higher value than before. This indicates some
effect of outlying observations on the non\hyp{}robust parameter estimation.
However the differences between robust and non\hyp{}robust methods are discussed
more thoroughly in Chapter \ref{chap:results_model_based}.
...
for each predictor a Bland-Altman Plot \citep{Bla86} is utilised to compare the 
...
correction has no large impact on the overall presentation.
...
paramteric bootstrap of Section \ref{sec:rfh_mse_boot} can be triggered when we
...
on the left hand side is similar to the one provided by @Mol15 in this example. 
Here we plot the coefficient of variation (CV) against a sorted sequence of 
domain identifiers. As in @Mol15 the presentation is restricted to values which 
have a smaller CV than 400. What we can observe is a clear benefit of using the 
REBLUP for domain predictions; here the *very unreliable* domains -- measured in
terms of CV -- benefit the most.
