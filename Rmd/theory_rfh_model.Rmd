...
$\pmat\beta$ and $\pmat\delta = (\rho_1, \sigma_1^2)$. Because the results are
needed in later sections the partial derivates of $\mat{V}(\pmat\delta)$ with
respect to each element in $\pmat\delta$ are given here: $\partial\mat{V} /
...
$\pmat\beta$ and $\pmat\delta = (\rho_1, \sigma_1^2, \rho_2, \sigma_2^2)$. The partial derivatives of $\mat{V}(\pmat\delta)$ with respect to each element in $\pmat\delta$ are given by: 
$\partial\mat{V} / \partial\sigma_2^2$ and $\partial\mat{V} / \partial\rho2$ are defined as for the temporal model; $\partial\mat{V} / \partial\sigma_1^2 = \mat{Z}_1 \pmat\Omega_1 \mat{Z}_1^\top$ and
\empty{
\begin{align*}
...
In principle I follow the results given by @Sin09 which is to start with the
...
Section \ref{sec:theory_sae_robust_mspe}. Following this idea the area level
...
$\mat{B}$. Note that $\mat{A}(\tilde{\pmat\beta}^\psi)\mat{y} =
\tilde{\pmat\beta}^\psi$ and that the specific form of $\mat{A}$ can be derived
starting from the estimation equation in
\eq{eq:rfhreeb}:
\empty{
\begin{align*}
...
\end{align*}
}which is similar to the form derived by @Cha14 for the robust BHF model - see
...
@Cha11 which is presented in Section \ref{sec:rfh_mse_pseudo_linear}.
...
$\psi$ the algorithm is non sensitive to the starting values with respect to 
...
$\mat{W}_2$ and $\mat{W}_3$ to one. Similar to the IRWLS algorithm before, if
the tuning constant of the influence function is chosen to be infinity the
algorithm has the desiarable property that it reduces to the form of the
non\hyp{}robust estimation equation of \eq{eq:rfheere} because of the special
form of $\mat{W}_2$ and $\mat{W}_3$, i.e. they reduce to identity matrices. And
- as it was reviewed in Section \ref{sec:robustee} - the solution to the
non\hyp{}robust equation \eq{eq:rfheere} is equivalent the the BLUP for
$\mat{u}$ given by \eq{eq:blupre}.
...
\eq{eq:reblupbhf}. The general approach is adapted for the robust area level 
models under study which is to derive fixed point equations for the variance 
parameters based on \eq{eq:rfhreed}. In order to stay as general as possible and
yet present simple solutions I want to distinguish between two cases. First when
$\pmat\delta$ is a scalar under a given model. Second when $\pmat\delta$ has
more than one element. This will lead to a form in which the solutions for the 
spatial and temporal extensions are just special cases. When we set $\pmat\delta
= \delta$, i.e. the first case, \eq{eq:rfhreed} can be written as:
\empty{
\begin{align*}
...
value - as long as they are chosen as values in the parameter space. Different
choices are studied in more detail in Section (?).

The solution by @Chat12 is based on the idea to add
...
\end{align*}
}Similar as before we can now state an iterative equation as:
\empty{
\begin{align*}
...
specifically the special form of $\bar{\mat{V}}_{ul}$ are derived when 
necessary. The FH model is ommitted since \eq{eq:fp_delta} is directly 
...
\mbox{}\newline In principle $\pmat\delta = (\rho_1, \sigma_1^2)$ under the spatial FH
model of Section \ref{sec:rfh_model_sfh}. Hence the specific form of
\eq{eq:fp_delta2} needs to be derived. In this case each $\bar{V}_{ul}$ with $l
= 1, 2$ need to be defined. Albeit it is possible to find such matrices the
implementation has lead to unstable results. This can be solved by splitting the
problem into finding solutions for $\delta_\sigma = (\sigma_1^2)$ and
$\delta_\rho = (\rho_1)$ seperately. This means that each iterative equation
depends on the other *known* parameter. This is numerically not optimal since it
will lead to an increase in overall iterations. However, as I will show later,
it leads to acceptable and stable results. For the specific adaption of the
iterative equations for $\pmat\delta$ this means that \eq{eq:fp_delta} is used
to find solutions for $\delta_\sigma$ and $\delta_\rho$ seperately and all other
specifications under the spatial model can be found in Section
...
been empirically validated the same numerical problems may be suspected when we
try to optimise the variance and correlation parameter jointly. Hence in this
...
assume known parameters if the left out elements.

For $\delta_\rho$ we can simply use the iterative equation \eq{eq:fp_delta}
which is directly applicable with the results of Section 
...
\mat{V}_u
  ...
  =& \left(\begin{matrix}\bar{\mat{V}}_{u1} & \bar{\mat{V}}_{u2} \end{matrix}\right)
    ...
    \end{matrix}\right)
\end{align*}
}and with the definition of $\bar{\mat{V}}_{u1}$ and $\bar{\mat{V}}_{u2}$ and in
conjunction with the results of Section \ref{sec:rfh_model_tfh} the iterative
...
\rho_2, \sigma_2^2)$. With the same reasoning as before we can split the problem
...
\bar{\pmat\Omega}_2$ and use the results of Section \ref{sec:rfh_model_stfh}.
...
we set $\bar{\mat{V}}_{u1} = \bar{\pmat\Omega}^*_1$ and $\bar{\mat{V}}_{u2} = 
\bar{\pmat\Omega}^*_2$ in conjunction with the results of Section
...
the approach suggested by @Sin09 and the reviewed literature of Section 
\ref{sec:robustee}. Of course the concrete implementations to find solutions for
$\pmat\beta$, $\pmat\delta$, and $\mat{u}$ are different.
...
correlation components of the variance\hyp{}coevariance structure as was layed
out in Section \ref{sec:rfh_solutions_delta}.
4. Set $m = m + 1$.
...
parameter vector so that when the algorithm stops all parameter values fulfill 
at least one of the two rules. In general the first stopping rule is used and it
is the relative absolute change. This is not feasible if the true parameter
value is 0, hence the distinction between the two cases. An advantage of these
rules is that they are scale independent which can be beneficial if the true
parameters have large absolute values as is pointed out by @Wei14[pp. 143-144].
A general discussion of algorithms in statistics can be found in @McC04 and more
specifically of robust regression estimates in @Mar06[pp. 325 ff].
