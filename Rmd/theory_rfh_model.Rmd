...
\end{matrix}
\right) \\ & + \frac{2\rho_2\pmat\Omega_{2i}}{1-\rho_2^2}.
\end{align*}
}
...
$\pmat\delta$ is a scala under a given model. Second when $\pmat\delta$ has more
than one element. This will lead to a form in which the solutions for the
...
0 = & 
  ...
  & - \tr\Paran{\underbrace{\mat{K}\mat{V}^{-1}\frac{\partial\mat{V}}{\partial\delta_l} (\mat{Z}\mat{V}_u\mat{Z}^\top)^{-1}}_{\mat{C}^*_{l}(\pmat\delta)}
  (\begin{matrix}\mat{Z}\bar{\mat{V}}_{u1}\mat{Z}^\top & \dots & \mat{Z}\bar{\mat{V}}_{uQ}\mat{Z}^\top\end{matrix}) \pmat\delta}
\end{align*}
...
In principle $\pmat\delta = (\rho_1, \sigma_1^2)$ under the spatial FH model of 
Section \ref{sec:rfh_model_sfh}. Hence the specific form of \eq{eq:fp_delta2}
needs to be derived. In this case each $\bar{V}_{ul}$ with $l = 1, 2$ need to be
defined. Albeit it is possible to find such matrices the implementation has lead
to unstable results. This can be solved by splitting the problem into finding
solutions for $\delta_\sigma = (\sigma_1^2)$ and $\delta_\rho = (\rho_1)$
seperately. This means that each iterative equation depends on the other *known*
parameter. This is numerically not optimal since it will lead to an increase in 
overall iterations. However, as I will show later, it leads to acceptable and 
stable results. For the specific adaption of the iterative equations for 
$\pmat\delta$ this means that \eq{eq:fp_delta} is used to find solutions for 
$\delta_\sigma$ and $\delta_\rho$ seperately and all other specifications under
the spatial model can be found in Section \ref{sec:rfh_model_sfh}.

#### Solutions for the Temporal Robust FH Model

Under the temporal model of Section \ref{sec:rfh_model_tfh} $\pmat\delta =
(\sigre, \rho_2, \sigma_2^2)$. Although not all combinations have been
empirically validated the same numerical problems may be suspected when we try
to optimise the variance and correlation parameter jointly. Hence in this case
the problem is split into finding solutions for $\pmat\delta_\sigma = (\sigre,
\sigma_2^2)$ and $\delta_\rho = (\rho_2)$. Both iterative equations assume
known parameters if the left out elements.
...
For the solutions under the spatio\hyp{}temporal model of Section
...
out in \ref{sec:rfh_solutions_delta}
4. Set $m = m + 1$.
