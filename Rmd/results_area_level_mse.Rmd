...
In addition to the prediction of a target statistic we are also interested in a 
measure of uncertainty with respect to this quantity. To this extent two MSPE 
estimators have been proposed in Section \ref{sec:theory_rfh_mse}: an adoption 
of the parametric bootstrap proposed by @Sin09 and an MSPE estimator based on
the pseudolinearisation approach by @Cha11. In what follows these two MSPE
estimators are compared in different simulation settings.

In the previously conducted simulation study we saw that the simulation setting 
...
underlying model. However the computational effort with respect to the bootstrap
estimator is relatively high which has made it necessary to restrict the number
of scenarios to a minimum. Thus for each model -- the RFH, RSFH, RTFH, and RSTFH
-- data is generated under the correct model and only a non\hyp{}outlier with an
outlier scenario are compared.

Also the results for the non\hyp{}robust methods are omitted to reduce the
...
corresponding review in Section \ref{sec:theory_sae_fh_extensions}. 

In the following Section \ref{sec:results_mse_scenarios} the simulation settings
for each model are described in detail. Section \ref{sec:results_mse_qm}
describes which measures are used to asses the performance of the MSPE
estimators. A presentation of the results can be found in Section
\ref{sec:results_mse_results}; followed by a discussion of the results in the
context of the existing literature in Section \ref{sec:results_mse_discussion}.
...
\sige$ and $\sige = \frac{4 (i - 1)}{D - 1} + 2$. Furthermore it is distinguished
between:
...
asses the quality of these estimators the relative root mean squared error
...
are made for the *current* time period, which is again defined as $t = T$ under
the respective scenario. This also means that we evaluate only the RMSPE for
these predictions. We can then define the RRMSE as:
$$
...
the interval. Also this effect was visible in the model\hyp{}based simulation in
Section \ref{sec:results_predictions}.
...
The performance of the MSPE estimators are overall promising. However the
...
when the prediction interval of the correction is too conservative. For the
...
predictor. This setting has been -- in contrast to Section
\ref{sec:results_predictions} -- avoided by choosing larger values for the
variance parameters in the above study.
...
outliers and scenarios without contamination. This result is very close to what 
