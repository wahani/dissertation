...
The Battese-Harter-Fuller (BHF) model, which can also be regarded as a linear mixed model, was introduced by @Bat88. The main focus of the present thesis 
...
for unit level models. In Chapter \ref{chap:rfh} these results are applied to 
the area level; here the basic unit level model and unit level notation is 
reviewed preparatory to more details in Section \ref{sec:saerobust} below. A
review of unit level models in the context of linear mixed models and EBLUP 
based predictions can be found in @Jia06 and also in @Rao03 and in @Rao15 who
also review a variety of extensions.
...
non-sampled part of the model. This representation makes it clear that a
fundamental assumption here is that the sampled units follow the population
model. This is plausible under simple random sampling within each domain and, as
@Rao15[79] note, also when $\mat{X}_S$ contains the variables used for sampling.
...
\end{align}
}where $\hat{\pmat\beta}$ denotes the BLUE of $\pmat\beta$ which is estimated using the
...
needs to be available to the analyst. The unknown parameters of the model, i.e. 
$\pmat\beta$, $\sigre$ and $\sigma_e^2$, can be estimated using standard ML and
REML estimation techniques and are based on the sampled data.
