...
Chapter (?) reveiwed the main advances in the SAE field with respect to two
...
income within muniscipalities. Furthermore they found that incorporating the
...
within domains and hence between the repeated measurements is adressed using the
...
Emperical results are here based on a model based simulation study. Later Sin05
showed the advantage of incorporating temporal autocorrelation into the analysis
where they predict the per capita comsumption expenditure using an annual
...
studies the advantage of such methods have been demontrated when outliers are
...
This connection is established in Chapter (?). Here it was distinguished between
three types of outlying observations: unit level outliers, area level outliers, 
and overly influencial observations. The first two types are common in the 
robust literature and describe outlying units or outlying areas respectively. 
The third type may be artificial in the context of area level models: they may 
present a problem when we smooth direct variance estimators. By doing this we 
may create observations which appear to be reliable but are in fact heavily 
influenced by unit level outliers. Especially with respect to area level
outliers and overly influencial observations the a robust extension to the FH
model may present itself to be beneficial.
...
The results of this Section show the overall beneficial effect of small area
...
associated to mixed linear models and Emperical Bayes methods \citep{Efr72}.
Interestingly we do not see any improvement using the bias correction technique;
especially in terms of bias. We also see an improvement by applying the robust
estimation technique in terms of MSPE.

In this study we have both, unit and area level outliers. It remains somewhat
intransparent if unit level outliers have an effect at all. In this example we
have a sampling design which is highly correlated with the target variable.
Hence we already adress the heterogeneity in the distribution of
tax\hyp{}turnover on the unit level since we incorporate the inclusion
propabilities. One property of the direct estimator -- also in the presence of
outliers -- is that we expect it to be design\hyp{}unbiased. If this property is
fullfilled then we should not expect to have an impact of asymmetric unit level
...
At present it is not clear how to adress this issue. It may be helpful to
incorporate the comparisson with the more established method by @Pra90 which at 
