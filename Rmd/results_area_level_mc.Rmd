...
and mean squared prediction error are compared over a variety of simulation 
...
the Monte Carlo sample directly from the area level model. This is a conceptual
contrast to the setting in Section \ref{sec:results_unit} in which we consider
the situation where we estimate the sampling variances as part of the study and 
thus need to generate the data on the unit level. However the area level 
perspective is considered in the literature to evaluate area level models -- see
for example @Fab10 and @Mar13 -- and hence is here considered first.
...
\ref{sec:rfh_framework} for a discussion of unit and area level outliers.
...
outliers. Although we will see some benefits of utilising spatial and temporal 
correlation structures the study does not aim to show the superiority of such 
methods but focuses on the effect of outliers instead. With respect to outliers
we only consider non\hyp{}symmetric outliers since they represent the
realistic use case. The choices of the simulation settings are presented in
Section \ref{sec:area_level_sim_setting} in detail; the results are then
presented in Section \ref{sec:area_level_sim_results}; followed by a discussion
...
considered. Similar to the approach by @Mar13 the FH model and spatial FH model 
are treated as special cases when we only use the data from the current time 
...
this setting it is possible to compare the different model specifications under 
a unified strategy.
...
- The random effect components are generated generally from the spatio-temporal 
...
Models with uncorrelated random effects are deduced when the respective
correlation parameter is set to zero. Since the impact of outliers is our main
concern only scenarios are considered where the spatial and temporal correlation
is set to $\rho = \rho_1 = \rho_2$ and the respective variance parameters
to $\sigma_u^2 = \sigma_1^2 = \sigma_2^2$. The concrete choices for $\rho$ and
$\sigma_u^2$ depend on the respective scenario and are defined below.
...
structure, i.e. $u_{1i} = u_{2it} = 0$ for these observations and replaced by
$u_i$.
...
direct estimator is denoted by *Direct*; it is simply the generated value for
$y_{iT}$.

To asses the quality of the predictions under the various methods two measures
are utilised: the relative bias (RBIAS) and relative root mean squared
...
consider the three observations we can make when studying the figures.
...
setting contribute to this effect. Identifying the spatial correlation 
...
or in this case correlation structure. This is also imminent in the comparison 
...
visible in Figure \ref{fig:stability_variance_spatio_temporal} where we observed
...
has changed which is due to the different nature of outlying areas; in Section
\ref{sec:saeRobust_stability} outliers are single observations, here outlying
areas include all observations over time. This results in the ability of the 
...
observation. As this is a surprising result a comparison has been conducted 
...
equations at their respective solutions reveal non\hyp{}satisfying results, i.e.
...
zero and hence only the regression estimates are used as a synthetic estimator.
...
As was discussed before in such a setting these models are over parametrised
...
explaining the difference is unknown at present. In various simulations the 
...
\ref{sec:results_unit} and somewhat beyond the scope of the discussion here.

The third observation is that the bias correction is not improving the results 
for the temporal models. The bias correction is constructing an interval around 
the inefficient but unbiased direct estimates in which we allow predictions to 
be made. The choice made in Section \ref{sec:rfhbiascorrection} for the size of
this interval may be too conservative in this simulation study. This effectively
leads to many repetitions in which domain predictions are unnecessarily bias
corrected. However, for the models which do not take the temporal structure into
account we can see that the bias correction has a beneficial effect on the
prediction of the outlying domains, both in terms of bias and consequently also
in terms of MSPE. For the non\hyp{}outlying areas we can see no additional gain
...
RBIAS and RRMSPE in the presence of outliers. Although not shown explicitly
...
can be made should be chosen with care in practice.
