...
@Cha86 and already reviewed in Section \ref{sec:sae_overview}. A thorough
discussion of what exactly outlying observations on the area level are will be given
in Section \ref{sec:rfh_framework} ahead.
...
- Schmid (?) spatial algorithmis fisher scoring

On the other hand @Schoch12 suggests finding solutions on 
...
- An outlier robust block bootstrap for small area estimation: ... & chambers
...
spatial REBLUP (SREBLUP). @Rao14 have extended the unit level mixed model to
...
@Bel06 and @Hua06 both use the t-distribution to model the random effect in the
  ...
  framework -- whereas the estimation methodology underpinning this Thesis is 
  based on Maximum Likelihood. @Ger10 and @Cha15 propose to use the FH model but
  replace the distribution of the random effects with a mixture distribution. 
  This essentially means that they have to detect which observations are 
  outliers and fit a seperate model. All these studies can claim some advantages
  for their respective method when outliers are present.
