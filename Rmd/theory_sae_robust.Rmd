...
Given the robust estimates $\hat{\beta}^\psi$, $\hat{\delta}^\psi$, and
$\hat{\mat{\re}}^\psi$ @Sin09 derive the REBLUP for area level means under the
BHF model \eq{eq:bhf}. The REBLUP is then a robust variation of \eq{eq:bhfeblup}
and can be stated as:
\empty{
\begin{align}
\label{eq:reblupbhf}
\hat{\theta}_i^{RBHF} = N_i^{-1} \Paran{\sum_{j \in S_i} y_{ij} + 
  \sum_{j\in R_i}\Paran{\sij{\mat{x}}^\top\hat{{\beta}}^\psi + \si{\hat{\re}^\psi}}}
\end{align}
}
...
observations than a symmetric distribution like a normal.
...
predicting the mean or median which with respect to bias introduces a problem
...
\end{align}
}where $\sij{\hat{e}}^\psi = \sij{y} - \sij{\mat{x}}^\top \hat{\beta}^\psi -
...
the robust estimation equations, $\psi_b$, with the important restriction that $c
> b$. This means that $\psi_c$ is still bounded but more liberal than $\psi_b$
which introduces the possibility to compute a potential prediction bias for each
area.
...
The estimation of the MSPE in model based SAE is one of the challenging problems in the discipline (cf. @Pfe13). This is only accelerated with the combination of robust estimation methodology so that @Sin09 proposed initially a bootstrap instead of a analytical solution. (?) have recently proposed a different bootstrap method targeting the problems of solutions like @Sin09. (?) proposed an analytical solution albeit this solution has some ceivats. 

In section \ref{sec:theory_sae_robust_bootstrap} existing bootstrap methods in this context are reviewed from which two are adapted for the later introduced models. Then in section \ref{sec:theory_sae_robust_mspe} one analytical solution is presented based on a pseudo linear representation of the REBLUP which also allows the estimation of the MSPE of the bias corrected REBLUP. Some of the results in chapter (?) are based on these estimators.
...
$\hat{\beta}^\psi$ and $\delta^\psi = \Paran{(\hat{\sigma}_u^2)^\psi,
(\hat{\sigma}_e^2)^\psi}$ samples are generated from the following model:
$$
\sij{y}^* = \sij{x}^\top \hat{\beta}^\psi + \re_i^* + \sij{e}^*
$$
...
\bar{Y}_i^{*} = N_i^{-1} \Paran{\sum_{j \in S_i} y^*_{ij} + 
  \sum_{j\in R_i}\Paran{\sij{\mat{x}}^\top\hat{{\beta}}^\psi + (1-n_iN_i^{-1})(\si{\re}^* + \bar{e}^*_i }}
$$
...
estimates $\hat{\delta} = (\hat{\sigma}_\re^2, \hat{\sigma}_e^2)$. @Sin09 argue
...
@Jio14 propose a different bootstrap method. This non-parametric approach is
  ...
  these populations bootstrap samples are drawn using the same sampling scheme
  ...
  for mixed linear models with correlated random effects which will be further
...
analytical solution for a bias\hyp{}robust mean squared error estimator for
small area estimators. The appeal of their method stems from the ability to be
used for a wide range of predictors. Specifically their method can be used for
any predictor which can be represented as weighted sum of the sampled response
vector. This includes all predictors discussed in the review of methods so far.
@Cha14 then targeted the REBLUP in \eq{eq:reblupbhf} and the bias corrected 
  version in \eq{eq:rbhfbc} among others and provide results for two different
  solutions: a method based on a pseudolinearisation of an predictor and a
  linearisation\hyp{}based approach.

The central idea is to represent the REBLUP as wigthed sum of the sampled
response values, $y_j$ with $j = 1, \dots, n$, which can be done for 
\eq{eq:reblupbhf} as follows:
$$
\si{\hat{\theta}}^{RBHF} = \sum_{j \in S} \sij{\mat{w}}^{RBHF} y_j = \Paran{\mat{w}_{iS}^{RBHF}}^\top \mat{y}_S
$$
where $\mat{y}_S$ denotes the vector of sampled response values and
$\mat{w}_{iS}^{RBHF}$ the vector of weights to produce the $i$th area
prediction. $\mat{w}_{iS}^{RBHF}$ is here defined as:
$$
\Paran{\mat{w}_{iS}^{RBHF}}^\top = N_i^{-1} \Paran{\mat{1}_{iS}^\top + (N_i - n_i) \Paran{\bar{\mat{x}}_{iR}^\top \mat{A}_S + \bar{\mat{z}}_{iR}^\top \mat{B}_S \Paran{\mat{I}_S - \mat{X}_S \mat{A}_S }}}
$$
...
selecting the random effects for area $i$. Here $\mat{1}_{iS}$ denotes a vector with n elements which are equal to one if the $j$th element is from area $i$ and zero otherwise; and $\mat{I}_S$ is a $n \times n$ identity matrix. Furthermore 
$$
...
$$
where $\mat{W}_{1S} = \Diag{\{w_{1j}\}_{j = 1}^n}_{n \times n}$ is a diagonal matrix with 
$$
w_{1j} = \psi\Paran{ \hat{U}_j^{-\frac{1}{2}} \Paran{ y_j - \mat{x}_j^\top\hat{\beta}^\psi } } 
\Paran{\hat{U}_j^{-\frac{1}{2}} \Paran{ y_j - \mat{x}_j^\top\hat{\beta}^\psi }}^{-1}
$$ as elements. Note that here $\hat{U}_j^{-\frac{1}{2}}$ denotes the $j$th diagonal element of the matrix $\mat{U}_S^{-\half}$. Continuing,
$$
  \mat{B}_s =
  \Paran{
...
$$
where $\mat{W}_{2S} = \Diag{\{w_{2j}\}_{j = 1}^n}_{n \times n}$ and $\mat{W}_{3S} = \Diag{\{w_{3i}\}_{i = 1}^D}_{D \times D}$ are diagonal matrices where their respective elements are defined by:
$$
  w_{2j} =
     \psi\Paran{\frac{1}{\hat{\sigma}^\psi_{e}} \Paran{y_i - x_i^\top \hat{\beta}^\psi - \hat{u}^\psi_i}}
     \Paran{\frac{1}{\hat{\sigma}^\psi_{e}} \Paran{y_i - x_i^\top \hat{\beta}^\psi - \hat{u}^\psi_i}}^{-1}
$$
and
$$
  w_{3i} = 
    \psi\Paran{\frac{ \hat{u}^\psi_i}{\hat{\sigma}_u^\psi} }
    \frac{ \hat{\sigma}_u^\psi }{\hat{u}^\psi_i}.
$$

@Cha14 then derive their MSPE estimator which can be formulated following the
  results of @Cha11 as:
$$
...
$$
a_{ij} = N_i w_{ij}^{RBHF} - I\Paran{j \in i}
$$
...
\widehat{\Exp{B}}\Paran{\si{\widehat{\theta}}^{RBHF}} =
  \sum_{j \in S} w_{ij}^{RBHF} \hat{\mu}_j - N_i^{-1} \sum_{j \in \Paran{R_i \cup S_i}} \hat{\mu}_j
$$
