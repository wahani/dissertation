...
For given parameters $\hat{\beta}^\psi$, $\delta^\psi = \Paran{(\hat{\sigma}_u^2)^\psi, (\hat{\sigma}_e^2)^\psi}$
$$
\sij{y}^* = \sij{x}^\top \hat{\beta}^\psi + \re_i^* + \sij{e}^*
$$
with $j = 1, \dots, n_i$ and $i = 1, \dots, D$ where $\re_i^* \sim \Distr{N}(0, (\hat{\sigma}_u^2)^\psi)$ and $\sij{e}^* \sim \Distr{N}(0, (\hat{\sigma}_e^2)^\psi)$
$$
\bar{Y}_i^{*} = N_i^{-1} \Paran{\sum_{j \in S_i} y^*_{ij} + 
  \sum_{j\in R_i}\Paran{\sij{\mat{x}}^\top\hat{{\beta}}^\psi + (1-n_iN_i^{-1})(\si{\re}^* + \bar{e}^*_i }}
$$
with $\si{\bar{e}}^* \sim \Distr{N}(0, (N_i - n_i)^{-1} (\hat{\sigma}_e^2)^\psi)$
