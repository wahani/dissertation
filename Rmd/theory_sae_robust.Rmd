...
In the above we have seen everything is great, awsome. But not everything is so super dupy as it appears. Predictions made using the methodology above are often asscociated with a bias. This is so because 
...
\end{align*}
}where $\sij{\hat{e}}^\psi = \sij{y} - \sij{\mat{x}}^\top \hat{\beta}^\psi - \hat{\re}_i^\psi$ is the prediction error associated to the REBLUP.
