...
re-weighted least squares (IRWLS) algorithm for a solution of \ref{eq:reeb}
...
\eq{eq:reed} and derives a stable algorithm - using a fixed point algorithm -
for $\delta$. 
...
the work by @Sin09. See @Ken80[chapter 11] and @Thi88[chapter 4] for
comprehensive overviews for solving ML based robust estimation equations, who
also suggest the IRWLS algorithm as an alternative to the NR algorithm. In
section (?) the explicit form and extension of these results are given for the
suggested estimators.

With given robust estimates for $\beta$ and $\delta$ @Sin09 solve the robust
mixed model equation \eq{eq:rmmere} proposed by @Fel86 and in order to derive a
robust EBLUP (REBLUP). Although they also suggest to use a NR algorithm which is
developed using a Taylor series expansion, the numerical solution appears to less problematic. This is indicated @Sch12 who 
