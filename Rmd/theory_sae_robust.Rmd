...
vector. @Cha14 then targeted the REBLUP in \eq{eq:reblupbhf} and the bias
corrected version in \eq{eq:rbhfbc} among others and provide results for two
different solutions: a method based on a pseudolinearisation of a predictor and
a linearisation\hyp{}based approach.

Following @Cha14 the RBLUP underlying \eq{eq:reblupbhf} can be represented as a
weigthed sum of the sampled response values as:
$$
\si{\tilde{\theta}}^{RBHF} = \sum_{j \in S} \sij{\mat{w}}^{RBHF} y_j = \Paran{\mat{w}_{iS}^{RBHF}}^\top \mat{y}_S
$$
...
$$ as elements. Note that here $U_j^{-\frac{1}{2}}$ denotes the $j$th diagonal element of the matrix $\mat{U}_S^{-\half}$. Continuing,
$$
  \mat{B}_s =
  \Paran{
...
@Cha14 then state the estimator of the MSPE following the results of @Cha11
  which in turn are based on the results of @Roy78. Hence the
$$
...
$$
is an estimator of the conditional prediction variance with $a_{ij} = N_i w_{ij}^{RBHF} - I\Paran{j \in i}$ and
$$
...
+ \sum_{k \in S}\phi^2_{kj}$ is a scaling constant. Due to the shrinkage effect
associated with EBLUPs @Cha11 suggest to use the unshrunken version of the EBLUP
under the BHF model for $\hat{\mu}_j$.
