...
(\hat{\sigma}_e^2)^\psi)$. Using each bootstrap sample, the REBLUP \eq{eq:reblupbhf} is computed and the suggested MSPE bootstrap estimator for $B$ repetitions is then defined by:
$$
\widehat{MSPE}\Paran{\hat{\theta}_i^{RBHF}} = \frac{1}{B} \sum_{l = 1}^B \Paran{\hat{\theta}_{il}^{RBHF} - \bar{Y}_{il}^{*}}^2
$$
