...
to address the potential bias of a robust prediction when we suspect
non\hyp{}symmetric outliers. @Cha14 and @Jio13 heve suggested different
corrections to the unit level REBLUP to take this prediction bias into account.
However the reviewed methods are based on the estimation of the within domain
prediction error; this approach is not directly feasible to area level model
...
different kinds of outliers we can observe on the area level. In Section 
...
regression estimator is a poor fit. Independet of this question @Fay79 refer to
@Efr71 and @Efr72 who argue that the Bayes Estimator and the Emperical Bayes
Estimator may improve the overall prediction performance but can be ill suited
for specific domains. In essence this also describes the situation in which area
level outliers are present in the data and we use a robust estimation procedure 
since we shrink the prediction twowards a global conditional mean. Although this
may improve the overall quality of the predictions it will not improve the
prediction for the outlying domains.
...
area level predictions using the known sampling variances. Hence we can modify
...
\sigma_{ei}$ to balance the tradeoff between loss in efficience and gain in
reduced bias. This approach can be directly applied to the spatial and temporal
extensions and thus similar to the area level REBLUP the area level REBLUP-BC in
\eq{eq:area_reblup_bc} acts as a plcaholder for all extensions.

Since we need the pseudo linear representation below, we can state
...
targeted by an area level correction without additional information. In Section 
(?) this issue is addressed again in a model based simulation study.
