...
SRSWOR where the domain specific samples sizes are: $n_i \in \{5, \dots, 15\}$.
...
sampling errors of the sample mean between 3 and 6 -- see Section (?) ahead for
a more detailed presentation of a similar scenario.
...
this setting we compute the sample mean vor the response variable and the single
...
configuring the repetetive elements across simulation studies with relative 
ease. However an important aspect of the pckage is the definition of the 
...
package. Hence we need to define these steps:
...
The obove code illustrates how new components can be defined. Notice that the 
...
because these components are easily understood. A main aspect contributing to
the readibility is that these definitions are decoupled from any control
structures.
...
`list` of `data.frame`s. These results are then combinsed into a single data
set:
...
configuered using the `R`-package `saeSim`. A design based configuration differs
...
comunicating the main idea of the composition of simulation studies. Available
...
this ragard the package `parallelMap` \citep{Bis15} is utilised as an interface
